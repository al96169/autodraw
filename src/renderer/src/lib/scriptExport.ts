...
function generateAnjian(ctx: ExportContext): string {
  ...
  strokes.forEach((s, i) => {
    ...
    if (options.moveSettleMs > 0) lines.push(`Delay ${options.moveSettleMs}`)
    lines.push('LeftDown 1')
    if (options.pressSettleMs > 0) lines.push(`Delay ${options.pressSettleMs}`)
...
function generateAhk(ctx: ExportContext): string {
  ...
  strokes.forEach((s, i) => {
    ...
    if (options.moveSettleMs > 0) lines.push(`Sleep, ${options.moveSettleMs}`)
    lines.push('Click, Down')
    if (options.pressSettleMs > 0) lines.push(`Sleep, ${options.pressSettleMs}`)
    for (let j = 1; j < s.points.length; j++) {
      const [px, py] = phys(s.points[j], region, sf)
...
function generatePython(ctx: ExportContext): string {
  ...
  lines.push(`move_settle = ${options.moveSettleMs} / 1000.0`)
  lines.push(`press_settle = ${options.pressSettleMs} / 1000.0`)
  lines.push('')
  ...
  strokes.forEach((s, i) => {
    ...
    if (options.moveSettleMs > 0) lines.push('time.sleep(move_settle)')
    lines.push("pyautogui.mouseDown(button='left')")
    if (options.pressSettleMs > 0) lines.push('time.sleep(press_settle)')
    for (let j = 1; j < s.points.length; j++) {
      const [px, py] = phys(s.points[j], region, sf)
